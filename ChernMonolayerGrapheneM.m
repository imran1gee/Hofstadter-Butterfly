...
for i = 1:1:length(kx)-1
    for j = 1:1:length(ky)-1
        ...
        nV3(:,:) = Vec(i+1,j+1,m:n,:);
        nV4(:,:) = Vec(i,j+1,m:n,:);
        UU1(i,j) =det((nV1)*ctranspose(nV2));
        if UU1(i,j)~=0  
           U1(i,j) = (UU1(i,j)/abs(UU1(i,j)));
        end
        UU2(i,j) =det((nV3)*ctranspose(nV1));
        if UU2(i,j)~=0
            U2(i,j) = (UU2(i,j)/abs(UU2(i,j)));
        end
        UU3(i,j) =det((nV4)*ctranspose(nV3));
        if UU3(i,j)~=0
           U3(i,j) = (UU3(i,j)/abs(UU3(i,j)));
        end
        UU4(i,j) =det((nV2)*ctranspose(nV4));
        if UU4(i,j)~=0
           U4(i,j) = (UU4(i,j)/abs(UU4(i,j)));
        end
        BF(i,j) =imag(log(U1(i,j)*U2(i,j)*U3(i,j)*U4(i,j)));
    end
end
Chern = 1*(sum(sum(BF(:,:))))/(2*pi);
